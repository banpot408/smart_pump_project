...
char auth_Debug[] = "NUhx2dXOg1egO7UKYppeh2ZMiIAatCLr"; //DN_ADD Debug
char ssid_Debug[] = "true_home2G_dfb"; //DN_ADD Debug
char pass_Debug[] = "NMcFfdxM"; //DN_ADD Debug
...
// long x, y;
  ...
  Serial.print("WiFi_Config comma_posi_3 : ");
  comma_posi_3 = WiFi_Config.indexOf(",", comma_posi_2+1);
  Serial.println(comma_posi_3);
  Serial.println(WiFi_Config.substring(comma_posi_2+1, comma_posi_3));  
  const String Blynk_Auth = WiFi_Config.substring(comma_posi_2+1, comma_posi_3);
...
Serial.print(" Blynk_Auth.length() : "); Serial.println(Blynk_Auth.length());
Serial.print(" ssid.length() : "); Serial.println(ssid.length());
Serial.print(" pass.length() : "); Serial.println(pass.length());


char auth_SD[Blynk_Auth.length()+1] ;
char ssid_SD[ssid.length()+1] ; 
char pass_SD[pass.length()+1] ;

Blynk_Auth.toCharArray(auth_SD, Blynk_Auth.length()+1);
ssid.toCharArray(ssid_SD, ssid.length()+1);
pass.toCharArray(pass_SD, pass.length()+1);

Serial.println(auth_SD);
Serial.println(ssid_SD);
Serial.println(pass_SD);

/* 
//    Blynk.begin(auth_Debug, ssid_Debug, pass_Debug, IPAddress(128,199,250,152), 8080);
//  Blynk.begin(auth_SD, ssid_SD, pass_SD, IPAddress(128,199,250,152), 8080);
...
*/  
  ...
  Serial.print(" auth_SD=>"); Serial.print(auth_SD); Serial.println("<= auth_SD");
  Serial.print(" ssid_SD=>"); Serial.print(ssid_SD); Serial.println("<= ssid_SD");
  Serial.print(" pass_SD=>"); Serial.print(pass_SD); Serial.println("<= pass_SD");
  
  Blynk.begin(auth_SD, ssid_SD, pass_SD);
//  Blynk.begin(auth_Debug, ssid_Debug, pass_Debug);
//  Blynk.begin(auth_Debug, ssid_Debug, pass_Debug, IPAddress(128,199,250,152), 8080);
  Serial.println("Blynk.begin() is Done! ");
